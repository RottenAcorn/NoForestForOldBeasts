...
/// </summary>
public class PoolingManager : NetworkBehaviour
{
    #region Singleton
    ...
    [SerializeField]
    public List<PooledObjectConfig> PooledPrefabsList;
    ...
    public void OnValidate()
    {
        for (var i = 0; i < PooledPrefabsList.Count; i++)
        {
            ...
            if (prefab != null)
            {
                Assert.IsNotNull(prefab.GetComponent<NetworkObject>(), $"{nameof(PoolingManager)}: Pooled prefab \"{prefab.name}\" at index {i.ToString()} has no {nameof(NetworkObject)} component.");
            }
        }
    }
...
[Serializable]
public struct PooledObjectConfig : IHandlerConfig
{
...
class PooledPrefabInstanceHandler : INetworkPrefabInstanceHandler
{
    ...
    public PooledPrefabInstanceHandler(GameObject prefab, PoolingManager pool)
    {
        _prefab = prefab;
    ...
    {
        return _poolingManager.GetNetworkObject(_prefab, position, rotation);
    }

    void INetworkPrefabInstanceHandler.Destroy(NetworkObject networkObject)
    {
        _poolingManager.ReturnNetworkObject(networkObject, _prefab);
    }
}
